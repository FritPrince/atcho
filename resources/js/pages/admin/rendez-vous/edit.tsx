...
export default function RendezVousEdit({ rendezVous }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        titre: rendezVous.titre,
        description: rendezVous.description,
        date_debut: rendezVous.date_debut ? new Date(rendezVous.date_debut).toISOString().slice(0, 16) : '',
        date_fin: rendezVous.date_fin ? new Date(rendezVous.date_fin).toISOString().slice(0, 16) : '',
        type: rendezVous.type,
        statut: rendezVous.statut,
        lieu: rendezVous.lieu,
        adresse_id: rendezVous.adresse_id?.toString() || '',
        createur_id: rendezVous.createur_id.toString(),
        participant_id: rendezVous.participant_id.toString(),
        projet_associe_id: rendezVous.projet_associe_id?.toString() || '',
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(`/admin/rendez-vous/${rendezVous.id}`);
    };

    return (
        <AdminLayout>
            <Head title={`Modifier - ${rendezVous.titre}`} />
            ...
            <div className="space-y-6">
                <div className="flex items-center space-x-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-bold tracking-tight">Modifier le rendez-vous</h1>
                        <p className="text-muted-foreground">
                            Modifiez les informations du rendez-vous "{rendezVous.titre}"
                        </p>
                    </div>
                </div>
