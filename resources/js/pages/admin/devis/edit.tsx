...
export default function DevisEdit({ devis }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        createur_id: devis.createur_id.toString(),
        atelier_id: devis.atelier_id.toString(),
        montant: devis.montant.toString(),
        statut: devis.statut,
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(`/admin/devis/${devis.id}`);
    };

    return (
        <>
            <Head title={`Modifier - Devis #${devis.id}`} />
                ...
                <div className="flex items-center space-x-4">
                    ...
                    <div>
                        ...
                        <p className="text-muted-foreground">
                            Modifiez les informations du devis #{devis.id}
                        </p>
                    </div>
                </div>
